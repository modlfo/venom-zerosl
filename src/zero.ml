...
module Primitives = struct
  ...
  let setRing output (ring : int) (value : int) : unit = RtMidi.sendMessageList output [ 176; 112 + ring - 1; value ]

  (** Sets the state of a button given the side (Left, Right), row (Top, Bottom) and column (1 - 8) *)
  let setButton output (side : side) (row : row) (column : int) (value : bool) : unit =
    let start =
      match side, row with
      | Left, Top -> 24
      | Left, Bottom -> 32
      ...
      | Right, Bottom -> 48
    in
    RtMidi.sendMessageList output [ 176; start + column - 1; (if value then 1 else 0) ]


  (** Sets the state of the button given the midi control number *)
  let setButtonRaw output (button : int) (value : int) : unit = RtMidi.sendMessageList output [ 176; button; value ]

  (** Clears the screens and buttons *)
  let clear output =
    RtMidi.sendMessageList output [ 240; 0; 32; 41; 3; 3; 18; 0; 4; 0; 2; 2; 1; 247 ] ;
    let rec loop n =
      if n > 8 then
        ()
      else
        let () = setRing output n 0 in
        let () = setButtonRaw output (24 + n) 0 in
        let () = setButtonRaw output (32 + n) 0 in
        let () = setButtonRaw output (40 + n) 0 in
        let () = setButtonRaw output (49 + n) 0 in
        loop (n + 1)
    in
    loop 0
  ...
  let init output : unit =
    RtMidi.sendMessageList output [ 240; 0; 32; 41; 3; 3; 18; 0; 4; 0; 1; 1; 247 ] ;
    clear output


  (** Low level writing of a label *)
  let writeLabelRaw output (str : string) (display : int) (position : int) : unit =
    ...
    RtMidi.sendMessageList output msg


  (** Writes a label given the side (Left, Right) the row (Top, Bottom) and column (1 - 8) *)
  let writeLabel output (str : string) (side : side) (row : row) (column : int) : unit =
    let display =
      match side, row with
      | Left, Top -> 1
      | Right, Top -> 2
    ...
    writeLabelRaw output msg display ((column - 1) * 9)
end
...
let decode (message : int list) : midi_message =
  ...
  if Array.length message = 3 then
    ...
    match data1 with
    | 176 when data2 >= 8 && data2 <= 15 -> KnobMsg (data2 - 7, data3)
    | 176 when data2 >= 16 && data2 <= 23 -> SliderMsg (data2 - 15, data3)
    | 176 when data2 >= 24 && data2 <= 31 -> BottonMsg ((Left, Top, data2 - 23), data3)
    | 176 when data2 >= 32 && data2 <= 39 -> BottonMsg ((Left, Bottom, data2 - 31), data3)
    | 176 when data2 >= 40 && data2 <= 47 -> BottonMsg ((Right, Top, data2 - 39), data3)
    | 176 when data2 >= 48 && data2 <= 55 -> BottonMsg ((Right, Bottom, data2 - 47), data3)
...
let paramName (parameter : parameter) : string = parameter.name
...
let percentFormat min max v = Printf.sprintf "%.2f" ((min +. (v *. (max -. min) /. 127.0)) *. 100.0) ^ " %"

let numberFormat min max v = Printf.sprintf "%.2f" (min +. (v *. (max -. min) /. 127.0))
...
let paramEncoder (parameter : parameter) : int =
  ...
  | Enum (_, n, index) -> index /. (n -. 1.0) *. 12.0 |> int_of_float
...
let paramInt (parameter : parameter) : int =
  ...
  | Enum (_, _, index) -> int_of_float index
...
let paramDisplay output (parameter : parameter) : unit =
  ...
  | Encoder column ->
      ...
      Primitives.setRing output column (paramEncoder parameter)
  | Knob column ->
      Primitives.writeLabel output parameter.name Left Top column ;
      Primitives.writeLabel output (paramString parameter) Left Bottom column
  | Slider column ->
  ...
  | Count (side, row, column) ->
      Primitives.writeLabel output parameter.name Left Top column ;
      Primitives.writeLabel output (paramString parameter) Left Bottom column ;
      Primitives.setButton output side row column parameter.pushed
  | PlusMinus ((side_p, row_p, column_p), (side_m, row_m, column_m)) ->
      Primitives.writeLabel output parameter.name Left Top column_p ;
      Primitives.writeLabel output (paramString parameter) Left Bottom column_p ;
      Primitives.setButton output side_p row_p column_p parameter.pushed ;
      Primitives.setButton output side_m row_m column_m parameter.pushed
  | Radio elems ->
      let index = paramInt parameter in
      Array.iteri (fun i (side, row, column) -> Primitives.setButton output side row column (i = index)) elems
  | Blank column ->
      Primitives.writeLabel output "" Left Top column ;
      Primitives.writeLabel output "" Left Bottom column ;
      Primitives.setRing output column 0
...
let mapSingle (group : group) (f : sub -> sub) (name : string list) =
  ...
  | Some (sub :: _) ->
      ...
      begin
        match mapGroup update (Group group) with
        | Group g -> Some g
        | _ -> None
      end
  | _ -> None
...
let applyMessageToParam (msg : midi_message) (modified : parameter list) (parameter : parameter) :
  ...
  | BottonMsg (msg_position, value), PlusMinus (plus, minus) ->
      if msg_position = plus then
        ...
        new_param :: modified, new_param
      else if msg_position = minus then
        let new_param = applyPlusMinusValue parameter (-value) in
        new_param :: modified, new_param
      else
        modified, parameter
  | BottonMsg (msg_position, _), Radio elems ->
      begin
        match array_find_index elems msg_position with
        | Some index ->
...
let compare_value (v1 : value) (v2 : value) : int =
  match v1, v2 with
  | Float (_, min1, max1, f1), Float (_, min2, max2, f2) -> compare (min1, max1, f1) (min2, max2, f2)
  | _ -> compare v1 v2
...
let compare_parameter (p1 : parameter) (p2 : parameter) : int =
  ...
  | 0 ->
      begin
        match compare p1.kind p2.kind with
...
let rec compare_group (g1 : group) (g2 : group) : int =
  ...
  | 0 ->
      let rec loop s1 s2 =
        ...
        | _, [] -> 1
      in
      loop (Array.to_list g1.subs) (Array.to_list g2.subs)
  | n -> n
*)
...
let countButton (name : string) (side : side) (row : row) (column : column) (elems : string array) (start : int) :
    parameter =
...
let plusMinusButton
  ...
  let start = float_of_int start -. 1.0 in
  { name
  ; kind = PlusMinus ((side_p, row_p, column_p), (side_m, row_m, column_m))
  ; value = Enum (elems, n, start)
  ; pushed = false
  ; group = None
  }
...
let update_state output (groups : group) (custom_handler : parameters -> group -> group) (message : midi_message) :
    group =
  let initial_groups = groups in
  let modified, groups = applyMessage message [] initial_groups in
  let groups = custom_handler (Array.of_list modified) groups in
  updateGroup output initial_groups groups ;
  groups


let handle_messages (group : group) input output custom_handler : unit -> unit =
  let state = ref group in
  fun () ->
    match RtMidi.getMessage input with
    | Some (_, []) -> ()
    | None -> ()
    | Some (_, data) ->
...
let openPort (name : string) (groups : group) (custom_handler : parameters -> group -> group) : t option =
  match RtMidi.openInput name, RtMidi.openOutput name with
  | Some input, Some output ->
      let f = handle_messages groups input output custom_handler in
      Primitives.init output ;
      forceUpdateGroup output groups ;
      Some f
  | Some input, None ->
      RtMidi.closeInput input ;
      None
  | None, Some output ->
      RtMidi.closeOutput output ;
      None
  | None, None -> None
